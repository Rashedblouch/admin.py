...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///baloch_master_final.db'
db = SQLAlchemy(app)
...
<style>
    ...
    .big-back-btn { 
        display: block; width: 90%; margin: 40px auto; padding: 30px; 
        background: #ff004c; color: #fff; font-size: 30px; font-weight: bold; 
        border-radius: 20px; text-decoration: none; border: 4px solid #fff;
    }
...
def handle_action(cmd):
    ...
    if cmd == "CHANGE ADMIN PASS":
        if request.method == 'POST':
            admin = AdminConfig.query.first()
            admin.password = request.form['p']
            db.session.commit()
            return "<h1>SUCCESS!</h1><a href='/dashboard' class='big-back-btn'>BACK</a>"
        return render_template_string(f'{STYLE}<div class="header"><h1>CHANGE PASS</h1></div><form method="POST"><input name="p" placeholder="NEW PASSWORD"><button type="submit" class="tool-btn" style="margin:auto;width:85%;">UPDATE</button></form><a href="/dashboard" class="big-back-btn" style="background:gray;">CANCEL</a>')

    return render_template_string(f'''{STYLE}
        <div style="height:100vh; background:#fff; color:#000; padding-top:100px;">
            <h1 style="font-size:45px;">COMMAND REGISTERED:</h1>
            <h2 style="color:red; font-size:55px;">{cmd}</h2>
            <p>Waiting for Target Website Connection...</p>
            <a href="/dashboard" class="big-back-btn">BACK TO DASHBOARD</a>
        </div>''')
